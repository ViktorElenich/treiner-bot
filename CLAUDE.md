...
**Устойчивость к сбоям Gemini:** Google периодически отвечает с серверов Render ошибкой `400 FAILED_PRECONDITION «User location is not supported»` — неверно определяет страну по IP хостинга, ошибка плавающая. Защита в два слоя: (1) `_call_gemini` в `content_gen.py` повторяет запрос до 4 раз с паузами 5/15/45 сек при временных ошибках (локация, 429, 500, 503, таймауты); (2) утренний автопост при неудаче сам повторяется через 30 минут, до 2 раз (`retries_left` в `send_daily_draft`, для ручного `/autopost` повторы отключены). Если ошибка станет постоянной — вероятно, IP региона Render попал в блок Google насовсем; тогда переносить сервис в другой регион Render (Oregon/Frankfurt) или включать платный тариф Gemini.
