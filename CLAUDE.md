...
Черновики хранятся в памяти (`_drafts`) — после редеплоя кнопки старых черновиков не работают, нужен `/autopost` заново.
...
Сейчас используется **тестовый** магазин ЮKassa. Переключение на боевые ключи — отдельный шаг, после подготовки контента для канала. Не переключай без явной просьбы Виктора.
