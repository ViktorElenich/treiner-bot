...
Заметки для будущих сессий Codex, работающих с `treiner-bot`.

Файл синхронизирован с `CLAUDE.md` — это одни и те же заметки для разных помощников.
Меняешь один файл — обнови второй, иначе следующий помощник будет работать по устаревшей картине.
...
- **БД:** PostgreSQL (Neon), адрес в `DATABASE_URL`. На SQLite не вернуться: файл на Render free tier стирается при каждом редеплое
